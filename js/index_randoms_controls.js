...
function normalizePercentRangeText(text) {
	// Convert "min - max%" to "min% - max%" for standard layout damage ranges.
	return text.replace(/(\d+(?:\.\d+)?)\s*-\s*(\d+(?:\.\d+)?)%/g, '$1% - $2%');
}
...
$(document)
	...
	.on("click.simplifiedbooststep", ".simplified-side-stat-step", function (ev) {
		...
		statRow.find(".simplified-side-stat-boost-input").val(String(nextBoost));
	});
