...
/*| Decimal Multiplier | Fixed-Point Value |
| ------------------ | ----------------- |
| 1.0×               | 4096              |
| 1.1×               | 4506              |
| 1.2×               | 4915              |
...
export function calculateSMSSSV(
  ...
  const hasPiercingContact = !!move.flags.contact &&
    attacker.hasAbility('Unseen Fist', 'Piercing Drill');
  const shouldNerfPiercingProtect = field.defenderSide.isProtected &&
    hasPiercingContact &&
    !!field.attackerSide.isPlayer;
  ...
  if (move.named('Weather Ball')) {
    ...
    const isMegaSolActive = attacker.hasAbility('Mega Sol');
    type =
      isMegaSolActive ? 'Fire'
    ...
    (move.named('Terrain Pulse') && isGrounded(attacker, field, field.attackerSide))
  ) {
    type =
  ...
  let isInfestate = false;
  let isGalvanize = false;
  let isLiquidVoice = false;
  let isEchoingAlloy = false;
  ...
  if (!move.isZ && !noTypeChange) {
    ...
    if ((isAerilate = attacker.hasAbility('Aerilate') && normal)) {
      ...
      type = 'Ice';
    } else if ((isInfestate = attacker.hasAbility('Infestate') && normal)) {
      type = 'Bug';
    } else if ((isNormalize = attacker.hasAbility('Normalize'))) { // Boosts any type
      type = 'Normal';
    }
    if (isGalvanize || isPixilate || isRefrigerate || isInfestate || isAerilate || isDragonize || isNormalize) {
      desc.attackerAbility = attacker.ability;
  ...
  attacker.hasAbility('Scrappy') || attacker.hasAbility('Reckless') || attacker.hasAbility('Mind\'s Eye') ||
  field.defenderSide.isForesight;
  const isPoisonRevealed =
  ...
  const isMegaSolWeatherBall = attacker.hasAbility('Mega Sol') && move.named('Weather Ball');
  if (!isMegaSolWeatherBall &&
  ...
  const basePowers = move.named('Triple Axel')
    ? Array.from({length: move.hits}, (_, hit) => calculateBasePowerSMSSSV(
      gen,
      attacker,
      defender,
      move,
  ...
  if (basePowers.some(basePower => basePower === 0)) {
    return result;
  }
  const basePower = basePowers[0];
  if (move.named('Triple Axel')) {
    desc.moveBP = Array.from({length: move.hits}, (_, hit) => 20 * (hit + 1)).join(' + ');
  }
  ...
  let baseDamages = basePowers.map(bp => getBaseDamage(attacker.level, bp, attack, defense));

  const isSpread = !field.ignoreSpreadDamageReduction && field.gameType !== 'Singles' &&
     ['allAdjacent', 'allAdjacentFoes'].includes(move.target);
  if (isSpread) {
    baseDamages = baseDamages.map(baseDamage => pokeRound(OF32(baseDamage * 3072) / 4096));
  }

  if (attacker.hasAbility('Parental Bond (Child)')) {
    baseDamages = baseDamages.map(baseDamage => pokeRound(OF32(baseDamage * 1024) / 4096));
  }
  ...
  if (isMegaSolActive && move.hasType('Fire')) {
    baseDamages = baseDamages.map(baseDamage => pokeRound(OF32(baseDamage * 6144) / 4096));
    desc.attackerAbility = attacker.ability;
  } else if (isMegaSolActive && move.hasType('Water')) {
    baseDamages = baseDamages.map(baseDamage => pokeRound(OF32(baseDamage * 2048) / 4096));
    desc.attackerAbility = attacker.ability;
  ...
  if (isCritical) {
    baseDamages = baseDamages.map(baseDamage => Math.floor(OF32(baseDamage * 1.5)));
  ...
  const applyFrostbite =
    ...
    move.category === 'Special' &&
    !attacker.hasAbility('Guts', 'Heart Boost') &&
    !move.named('Facade');
  ...
  const damageByHit = baseDamages.map(baseDamage => {
    const hitDamage: number[] = [];
    for (let i = 0; i < 16; i++) {
      hitDamage[i] = getFinalDamage(
        baseDamage,
        i,
        typeEffectiveness,
        applyBurn,
...
export function calculateBasePowerSMSSSV(
  ...
  case 'Weather Ball':
    const isMegaSolActive = attacker.hasAbility('Mega Sol');
    basePower = move.bp * ((isMegaSolActive || (field.weather && !field.hasWeather('Strong Winds'))) ? 2 : 1);
    if (!isMegaSolActive &&
      field.hasWeather('Sun', 'Harsh Sunshine', 'Rain', 'Heavy Rain') &&
      attacker.hasItem('Utility Umbrella')) basePower = move.bp;
    if (isMegaSolActive) desc.attackerAbility = attacker.ability;
    desc.moveBP = basePower;
    break;
  case 'Terrain Pulse':
    basePower = move.bp * (
      isGrounded(attacker, field, field.attackerSide) && field.terrain ? 2 : 1
    );
    desc.moveBP = basePower;
    break;
  case 'Rising Voltage':
    basePower = move.bp * ((isGrounded(defender, field, field.defenderSide) &&
      field.hasTerrain('Electric')) ? 2 : 1);
    desc.moveBP = basePower;
    break;
  case 'Fling':
  ...
  case 'Triple Axel':
    basePower = basePowerOverride ?? (move.hits === 2 ? 30 : move.hits === 3 ? 40 : 20);
    desc.moveBP = basePower;
    break;
...
export function calculateBPModsSMSSSV(
  ...
  let resistedKnockOffDamage =
    ...
    (defenderName.includes('Giratina-Origin') && defenderItem.includes('Griseous')) ||
    (defenderName.includes('Arceus') && defenderItem.includes('Plate')) ||
    (defenderName.includes('Genesect') && defenderItem.includes('Drive')) ||
    ...
    (defenderName.includes('Silvally') && defenderItem.includes('Memory')) ||
    ...
    (defenderName.includes('Ogerpon-Cornerstone') && defender.hasItem('Cornerstone Mask')) ||
    (defenderName.includes('Ogerpon-Hearthflame') && defender.hasItem('Hearthflame Mask')) ||
    (defenderName.includes('Ogerpon-Wellspring') && defender.hasItem('Wellspring Mask')) ||
  ...
  if ((move.named('Facade') && attacker.hasStatus('brn', 'par', 'psn', 'tox', 'frb')) ||
    ...
    move.named('Expanding Force') &&
      isGrounded(attacker, field, field.attackerSide) && field.hasTerrain('Psychic')
  ) {
    ...
    (move.named('Misty Explosion') &&
      isGrounded(attacker, field, field.attackerSide) && field.hasTerrain('Misty')) ||
    ...
    const type1Effectiveness = getMoveEffectiveness(
      ...
      isGhostRevealed,
      undefined,
      isGrounded(defender, field, field.defenderSide),
      isRingTarget
    );
  ...
  if ((attacker.hasAbility('Technician') && basePower <= 60) ||
    ...
    (attacker.hasAbility('Sharpness', 'Surge Cutter') && move.flags.slicing)
  ) {
    bpMods.push(6144);
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (attacker.hasItem(`${move.type} Gem`)) {
    ...
    (attackerName.includes('Ogerpon-Wellspring') && attacker.hasItem('Wellspring Mask'))
  ) {
    bpMods.push(4915);
    desc.attackerItem = attacker.item;
  } else if (
...
export function calculateAtModsSMSSSV(
  ...
  if ((attacker.hasAbility('Slow Start') && attacker.abilityOn &&
    ...
    attacker.hasAbility('Electromorphosis') &&
    attacker.abilityOn &&
    move.hasType('Electric')
  ) {
    atMods.push(8192);
  ...
  const attackerProtoQuarkBoostedStat = getProtoQuarkBoostedStat(attacker, field);
  if (
    (move.category === 'Physical' && attackerProtoQuarkBoostedStat === 'atk') ||
    (move.category === 'Special' && attackerProtoQuarkBoostedStat === 'spa')
  ) {
    atMods.push(5325);
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (
    (attacker.hasAbility('Hadron Engine') && move.category === 'Special' &&
      field.hasTerrain('Electric') && isGrounded(attacker, field, field.attackerSide)) ||
...
export function calculateDfModsSMSSSV(
  ...
  const defenderProtoQuarkBoostedStat = getProtoQuarkBoostedStat(defender, field);
  if (
    (hitsPhysical && defenderProtoQuarkBoostedStat === 'def') ||
    (!hitsPhysical && defenderProtoQuarkBoostedStat === 'spd')
  ) {
    desc.defenderAbility = defender.ability;
    dfMods.push(5324);
  }
...
export function calculateFinalModsSMSSSV(
  ...
  if (defender.hasAbility('Multiscale', 'Shadow Shield') &&
      ...
      (!field.defenderSide.spikes || !isGrounded(defender, field, field.defenderSide)) &&
